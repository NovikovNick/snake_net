...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR,
  ...
  if (wcscmp(__wargv[offset], L"spectate") == 0) {
    ...
    if (swscanf_s(__wargv[offset + 1], L"%[^:]:%hu", wide_ip_buffer,
                  wide_ip_buffer_size, &host_port) != 2) {
      Syntax();
      return 1;
    }
    ...
    for (i = 0; i < num_players; i++) {
      ...
      if (swscanf_s(arg, L"%[^:]:%hd", wide_ip_buffer, wide_ip_buffer_size,
                    &players[i].u.remote.port) != 2) {
        Syntax();
        return 1;
      }
    ...
    while (offset < __argc) {
      ...
      if (swscanf_s(__wargv[offset++], L"%[^:]:%hd", wide_ip_buffer,
                    wide_ip_buffer_size, &players[i].u.remote.port) != 2) {
        Syntax();
        return 1;
      }
      wcstombs_s(nullptr, players[i].u.remote.ip_address,
